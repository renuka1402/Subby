...
const LoginScreen = ({ navigation }) => {
  ...
  const validateAndLogin = async () => {
    if (!email.trim() || !password.trim()) {
      return Alert.alert('Validation', 'Email और Password दोनों भरिए');
    }
    ...
    try {
      ...
      const { token, Contractors = [] } = res.data.data || {};
      const companyId = Contractors[0]?.company_id;
      ...
      await AsyncStorage.setItem('userEmail', lowerCaseEmail);
      if (companyId != '') {
